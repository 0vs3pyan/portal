...
def sort(request, queryset, order):
    ...
    elif request.session['var'] == 'Квартира' and request.session['number_of_rooms'] == 'Квартира':
        all_flat = queryset.objects.filter(flat_or_apartment='Квартира').order_by(order)
    elif request.session['var'] == 'Дом' and request.session['number_of_rooms'] == 'Дом':
        all_flat = queryset.objects.filter(flat_or_apartment='Квартира').order_by(order)

    elif request.session['var'] == 'Квартира' and request.session['number_of_rooms'] == 'zero':
        all_flat = queryset.objects.filter(flat_or_apartment='Квартира').order_by(order)
    elif request.session['var'] == 'Дом' and request.session['number_of_rooms'] == 'zero':
